...
const cameraFormatStatus = {
  OK: binding.BARE_CAMERA_FORMAT_OK,
  PENDING: binding.BARE_CAMERA_FORMAT_PENDING,
  ERROR: binding.BARE_CAMERA_FORMAT_ERROR
}
...
class SDLCameraSpec {
  ...
  get status() {
    const status = binding.getCameraSpecStatus(this._spec)

    switch (status) {
      case cameraFormatStatus.OK:
        return 'ok'
      case cameraFormatStatus.PENDING:
        return 'pending'
      case cameraFormatStatus.ERROR:
        return 'error'
      default:
        return 'error'
    }
...
class SDLCamera {
  ...
  static defaultCamera(spec) {
    ...
    if (!spec) {
      const formats = SDLCamera.getSupportedFormats(devices[0])
      spec = formats[0]
    }

    return new SDLCamera(devices[0], spec)
  }
  ...
  static getCameras() {
    ...
    return cameras.map((id, index) => {
      const name = SDLCamera.getCameraName(id)
  ...
  get format() {
    const spec = binding.getCameraFormat(this._handle)
    return new SDLCameraFormat(spec)
  }
